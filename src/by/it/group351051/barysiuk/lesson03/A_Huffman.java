package by.it.group351051.barysiuk.lesson03;
...
public class A_Huffman {
    ...
    String encode(File file) throws FileNotFoundException {
        ...
        while (priorityQueue.size() > 1) {
            Node left = priorityQueue.poll();  // Извлекаем узел с минимальной частотой
            Node right = priorityQueue.poll(); // Извлекаем следующий узел с минимальной частотой
            InternalNode parent = new InternalNode(left, right); // Создаем родительский узел
            priorityQueue.add(parent); // Добавляем его обратно в очередь
        }

        //4. последний из родителей будет корнем этого дерева
        Node root = priorityQueue.poll();
        if (root != null) {
            root.fillCodes(""); // Генерируем коды Хаффмана для всех символов
        }

        StringBuilder sb = new StringBuilder();
        for (char c : s.toCharArray()) {
            sb.append(codes.get(c)); // Получаем код для символа из карты codes
        }
    ...
    public static void main(String[] args) throws FileNotFoundException {
        ...
        File f = new File(root + "dataHuffman.txt");
